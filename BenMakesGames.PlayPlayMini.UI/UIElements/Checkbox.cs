...
public class Checkbox : IUIElement
{
    ...
    private SpriteSheet CheckBox => UI.Graphics.SpriteSheets[UI.Theme.Theme.CheckboxSpriteSheetName];

    public IUIElement.ClickDelegate DoDoubleClick { get; set; }
    public IUIElement.ClickDelegate DoClick => (int x, int y) => { Checked = ToggleHandler(!Checked); };
